-- POS sync structures (idempotent, fixed constraints via DO blocks)
...
do $$ begin
  if not exists (
    ...
    where c.contype = 'u'
      and c.conname = 'pos_products_uniq_location_provider_external'
      and n.nspname = 'public'
      and t.relname = 'pos_products'
  ) then
    alter table public.pos_products
      add constraint pos_products_uniq_location_provider_external
...
do $$ begin
  if not exists (
    ...
    join pg_namespace n on n.oid = t.relnamespace
    where c.contype = 'u'
      and c.conname = 'pos_orders_uniq_location_provider_external'
      and n.nspname = 'public'
      and t.relname = 'pos_orders'
  ) then
    alter table public.pos_orders
      add constraint pos_orders_uniq_location_provider_external
      unique (location_id, provider, external_id);
  end if;
end $$;
