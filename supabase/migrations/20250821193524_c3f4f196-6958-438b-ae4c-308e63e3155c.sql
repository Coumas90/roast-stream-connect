-- ===============================================
-- Card 13: POS Dashboard + MTTR Analytics (Fixed)
-- ===============================================
...
-- Simple index without predicate (immutable function requirement)
...
BEGIN
  ...
  WHERE hours_until_expiry < 24 AND expires_at > now();

  -- Count warning expirations (< 72 hours)
  SELECT COUNT(*) INTO v_expirations_warning
  FROM public.pos_dashboard_expirations
  WHERE hours_until_expiry < 72 AND hours_until_expiry >= 24 AND expires_at > now();
