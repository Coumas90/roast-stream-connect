-- Retry migration with immutable-safe partial index
...
-- 2) Foreign key for location_id (only if missing)
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  IF NOT EXISTS (
    ...
    -- Note: predicate must be immutable; we only use accepted_at IS NULL
...
-- 6) RPCs (same as previous attempt)
