-- 1) Ensure pgcrypto for SHA-256 hashing
...
-- Only one open invitation per (tenant,email,role)
...
create policy if not exists invitations_select_admin_only
on public.invitations for select
using (public.is_tupa_admin());

create policy if not exists invitations_write_admin_only
