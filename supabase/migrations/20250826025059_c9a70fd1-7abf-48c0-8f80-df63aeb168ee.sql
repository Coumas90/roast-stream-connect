-- Performance optimizations to fix timeout issues
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_roles_user_tenant ON public.user_roles(user_id, tenant_id) WHERE tenant_id IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_roles_user_location ON public.user_roles(user_id, location_id) WHERE location_id IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_locations_tenant_id ON public.locations(tenant_id);

-- 2. Optimize the most critical authorization functions with better queries
...
-- 4. Add specific optimization for locations queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_locations_name_tenant ON public.locations(name, tenant_id);

-- 5. Optimize RLS policies by simplifying complex conditions
-- Update locations RLS policy to be more efficient
DROP POLICY IF EXISTS "locations_select_by_access" ON public.locations;
CREATE POLICY "locations_select_by_access" 
ON public.locations 
FOR SELECT 
USING (
  -- Use optimized function call
