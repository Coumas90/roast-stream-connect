...
BEGIN
  ...
  WHERE scope = 'proactive_alert'
    AND (meta->>'alert_id')::text = _alert_id
  ORDER BY ts DESC
  LIMIT 1;
...
SELECT 
  (pl.meta->>'alert_id')::text as alert_id,
  ar.name as alert_name,
  ar.alert_type,
  COUNT(*) as trigger_count,
  MAX(pl.ts) as last_triggered,
  MIN(pl.ts) as first_triggered,
  AVG(EXTRACT(epoch FROM (LEAD(pl.ts) OVER (PARTITION BY (pl.meta->>'alert_id')::text ORDER BY pl.ts) - pl.ts)) / 60) as avg_frequency_minutes
FROM public.pos_logs pl
JOIN public.alert_rules ar ON ar.id = (pl.meta->>'alert_id')::text
WHERE pl.scope = 'proactive_alert'
  AND pl.meta->>'alert_id' IS NOT NULL
GROUP BY (pl.meta->>'alert_id')::text, ar.name, ar.alert_type
