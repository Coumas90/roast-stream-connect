...
--     Si prefieres manual: reemplaza <UUID_REAL> debajo.
WITH any_user AS (
...
-- 8) Vistas del dashboard: security_invoker + grants
SELECT 
  v.schemaname, v.viewname,
  COALESCE( (SELECT bool_or((o."option"='security_invoker') AND (o.setting='true'))
             FROM pg_options_to_table(c.reloptions) o), false) AS security_invoker_on
FROM pg_class c
JOIN pg_namespace n ON n.oid = c.relnamespace
JOIN pg_views v ON v.viewname = c.relname AND v.schemaname = n.nspname
WHERE n.nspname = 'public'
  AND v.viewname IN ('pos_dashboard_expirations','pos_dashboard_breakers');
